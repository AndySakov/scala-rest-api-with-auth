...
//   */
//  override def find(loginInfo: LoginInfo): Future[Option[PasswordInfo]] = Future.successful(userDAO.find(loginInfo).result.data.map(_.passwordInfo))
...
//   * @param passwordInfo user's hashed password
//   */
//  override def update(loginInfo: LoginInfo, passwordInfo: PasswordInfo): Future[PasswordInfo] = userDAO.find(loginInfo).result.data.flatMap {
//    case Some(user) => userDAO.update(user.copy(password = Some(passwordInfo.password))).result.data.map(_.passwordInfo)
